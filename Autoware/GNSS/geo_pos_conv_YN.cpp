...
void geo_pos_conv::set_plane(int num)
{
  ...
  else if (num == 8)  //library_basement
  {
    lon_deg = 22.999977;  
    lon_min = 0;
    lat_deg = 120.216793;
...
void geo_pos_conv::conv_llh2xyz(void)
{
  ...
  //similar to ncku shuei li convertor
  double a,b,lon0,k0,dx,e,e2,n,nu,p,A,B,C,D,E,S,K1,K2,K3,K4,K5;   /////////////////////////////////////////////////////////////////////////////////
  //phi = m_lat;
	//lambda = m_lon;

  //parameter of wgslla2twd97xyz

  a = 6378137.0;  // earth semimajor axis in meters
  b = 6356752.314245;  // earth semiminor axis in meters	
  lon0 = 121 * M_PI / 180;
  k0 = 0.9999;
  dx = 250000;
  ...
  //-----------------------------------------------------------------------------------------------------------------------
	...
	e = std::sqrt(1-(b/a)*(b/a));
  e2 = std::pow(e,2)/(1-std::pow(e,2));
  n = ( a - b ) / ( a + b );
  nu = a / std::pow((1-(std::pow(e,2)) * (std::pow(std::sin(m_lat), 2) ) ) , 0.5);
  p = m_lon - lon0;

  A = a * (1 - n + (5/4) * (std::pow(n,2) - std::pow(n,3)) + (81/64) * (std::pow(n, 4)  - std::pow(n, 5)));
  B = (3 * a * n/2.0) * (1 - n + (7/8.0)*(std::pow(n,2) - std::pow(n,3)) + (55/64.0)*(std::pow(n,4) - std::pow(n,5)));
  C = (15 * a * (std::pow(n,2))/16.0)*(1 - n + (3/4.0)*(std::pow(n,2) - std::pow(n,3)));
  D = (35 * a * (std::pow(n,3))/48.0)*(1 - n + (11/16.0)*(std::pow(n,2) - std::pow(n,3)));
  E = (315 * a * (std::pow(n,4))/51.0)*(1 - n);
  S = A * m_lat - B * std::sin(2 * m_lat) +C * std::sin(4 * m_lat) - D * std::sin(6 * m_lat) + E * std::sin(8 * m_lat);

  //compute X value
  K1 = S*k0;
  K2 = k0*nu*std::sin(2*m_lat)/4.0;
  K3 = (k0*nu*std::sin(m_lat)*(std::pow(std::cos(m_lat),3))/24.0) * (5 - std::pow(std::tan(m_lat),2) + 9*e2*std::pow((std::cos(m_lat)),2) + 4*(std::pow(e2,2))*(std::pow(std::cos(m_lat),4)));
  m_x = K1 + K2*(std::pow(p,2)) + K3*(std::pow(p,4));

  //compute Y value
  K4 = k0*nu*std::cos(m_lat);
  K5 = (k0*nu*(std::pow(std::cos(m_lat),3))/6.0) * (1 - std::pow(std::tan(m_lat),2) + e2*(std::pow(std::cos(m_lat),2)));
  m_y = K4 * p + K5 * (std::pow(p, 3)) + dx;

  //compute Z value(same as GPS's ellipsoid height)
	m_z = m_h;     ///////////////////////////////////////////(already add undulation in m_h)
...
{
  geo_pos_conv demo;
  double lon = 120.147275611372, lat = 23.0510545102663, ele = 20.0;
  int plane = 7;

  demo.set_plane(plane);
  demo.llh_to_xyz(lat, lon, ele);

  printf("WGS84: lon = %.8lf, lat = %.8lf\n", lon, lat);
  printf("TWD97: x = %.8lf, y = %.8lf\n", demo.x(), demo.y());
